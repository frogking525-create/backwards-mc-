...
lines are marked `// MAPPING CHECKPOINT` in the source for exactly this reason. Two of them
(`respawnPosition()` and `server()` in `BackwardsChallengeMod.handleRespawn()`) are still
educated guesses rather than compiler-confirmed, since the real 26.2 build error only told us
`getRespawnPosition()`/`getServer()` don't exist anymore, not what replaced them - if either of
those two specific lines fails, open `ServerPlayer`/`Entity` in your IDE and search for the
current name of "get the player's bed/anchor position" or "get the MinecraftServer this entity
belongs to", and swap the name in. Everything else marked MAPPING CHECKPOINT is either
confirmed-correct against a real compile, or affects only a non-critical flourish (the vanilla
credits packet, the explosion visual) - if compilation fails there, deleting that one call (or
the try/catch around it) is enough; the rest of the mod still works.
